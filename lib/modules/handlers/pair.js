...
module.exports = {
    ...
    get: function (request, reply) {
        ...
        redis.get('conf:' + code, (err, result) => {
            ...
            if (result) {
                ...
                redis.set('b2m:' + encryptionService.hashKey(browserUuid), encryptionService.hashKey(result));
                redis.set('m2b:' + encryptionService.hashKey(result), encryptionService.hashKey(browserUuid));
    ...
    delete: function (request, reply) {
        ...
        // try to get the pairing code that is matched 
        redis.get('m2b:' + encryptionService.hashKey(uuid), (err, result) => {

                // set a new flag for the originating device (i.e. browser) to know that pairing was successful
            redis.del('b2m:' + result);
            redis.del('m2b:' + encryptionService.hashKey(uuid));
            
            log.info('deleted pairing relationship between hashed uuid ' + encryptionService.hashKey(uuid) + ' with uuid ' +  result);
